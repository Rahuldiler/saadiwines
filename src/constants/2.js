const template2InfoData = {
  milestone: [
    {
      id: 1,

      title:
        "What is your idea of a perfect lazy day spent together as a couple?",
      description:
        "A perfect lazy day spent together as a couple would involve sleeping in, enjoying a leisurely breakfast, and snuggling up for a movie marathon. We would order takeout for lunch, engage in light activities like puzzles or reading, and end the day with a cozy homemade dinner. It would be a day of relaxation, quality time, and blissful simplicity.",
    },
    {
      id: 3,

      title: "Your dream honeymoon destination?",
      description:
        "My dream honeymoon destination would be the breathtaking Maldives. Picture-perfect white sandy beaches, crystal-clear turquoise waters, and luxurious overwater bungalows create the ultimate romantic setting. We could spend our days lounging by the private pool, indulging in spa treatments, and exploring the vibrant marine life through snorkeling or scuba diving. The Maldives offers a serene and idyllic paradise for a truly unforgettable honeymoon experience.",
    },
    {
      id: 4,

      title:
        "Share a funny or endearing nickname you have for your partner and why?",
      description:
        "One funny nickname I have for my partner is 'Snuggle Muffin.' It originated from our love for cuddling and being affectionate with each other. Whenever we cuddle, it's like sinking into a warm, cozy muffin of love. It always brings a smile to our faces and reminds us of the sweet and affectionate moments we share together.",
    },
    {
      id: 2,

      title: "What is your dream destination for your bachelors and why?",
      description:
        "I would choose Ibiza. Known as the party capital of the world, Ibiza offers a thrilling and unforgettable experience. With its world-famous clubs, stunning beaches, and electric atmosphere, it's the perfect destination to celebrate with friends. We would dance to the beats of renowned DJs, soak up the sun on the pristine beaches, and enjoy the vibrant nightlife. Ibiza's unique blend of music, relaxation, and adventure would create lifelong memories for an epic bachelor party.",
    },
  ],
  familyMembers: [
    {
      id: 1,
      name: "Sudev",
      relation: "Father",
    },
    {
      id: 2,
      name: "Geeta",
      relation: "Mother",
    },
    {
      id: 3,
      name: "Rajesh",
      relation: "Uncle",
    },
    {
      id: 4,
      name: "Archana",
      relation: "Sister",
    },
    {
      id: 5,
      name: "Govind",
      relation: "Brother",
    },
    {
      id: 6,
      name: "Vishal",
      relation: "Brother",
    },
    // {
    //   id: 7,
    //   name: "Rupali",
    //   relation: "Sister",
    // },
    // {
    //   id: 8,
    //   name: "Vedika",
    //   relation: "Sister",
    // },
    // {
    //   id: 9,
    //   name: "Sri",
    //   relation: "Sister",
    // },
    // {
    //   id: 10,
    //   name: "Ravi",
    //   relation: "Maternal uncle",
    // },
    // {
    //   id: 11,
    //   name: "Abhay",
    //   relation: "Maternal uncle",
    // },
    // {
    //   id: 12,
    //   name: "Ronak",
    //   relation: "Cousin",
    // },
  ],
  itinerary: [
    {
      id: 1,

      functionName: "The Vibrant Mehendi Ceremony",
      details:
        "Delve into the captivating realm of the Mehendi ceremony, where the artistry of intricate henna designs comes to life, adorning the hands and feet of the radiant bride. Each delicate pattern tells a story, symbolizing the boundless realms of love, the pursuit of eternal happiness, and the abundant blessings of fertility.",
      dateTime: "2024-06-18T08:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/Indira+Gandhi+International+Airport/@28.527554,77.0438314,11z/data=!4m6!3m5!1s0x390d1b85fc2a2d89:0xbef376182c43ed9d!8m2!3d28.5563529!4d77.1006963!16zL20vMDEyNDQ0?entry=ttu",
      image: "string",
    },
    {
      id: 2,
      functionName: "Joyful Sangeet Celebration",
      details:
        "Immerse yourself in the vibrant and exhilarating ambiance of the Sangeet ceremony, where the air resonates with the harmonies of music and the rhythmic beats of dancing feet. The stage comes alive with spellbinding performances, as both families unite to honor and rejoice in the union of two souls. ",
      dateTime: "2024-06-18T09:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/Mazaar+Restaurant/@28.5688605,77.2328845,15z/data=!4m6!3m5!1s0x390ce3ad97db2cb5:0xdce447161ff5833!8m2!3d28.5701715!4d77.2443807!16s%2Fg%2F1v7pwvx2?entry=ttu",
      image: "string",
    },
    {
      id: 3,

      functionName: "Grandeur of Baraat",
      details:
        "Experience the exuberance of the Baraat procession as the groom, surrounded by loved ones, makes his grand entrance to the wedding venue. The air pulsates with infectious energy, music fills the atmosphere, and joyful dancing sets the rhythm for an unforgettable celebration of love and unity.",
      dateTime: "2024-06-18T08:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/C+R+Park+Market+No.1/@28.5350621,77.2431712,13.75z/data=!4m6!3m5!1s0x390ce3d83e0cb4b7:0xb7ece1334b216b84!8m2!3d28.5401153!4d77.2486233!16s%2Fg%2F1wk4bfx6?entry=ttu",
      image: "string",
    },
    {
      id: 4,

      functionName: "Sacred Seven Steps",
      details:
        "Unearth the profound significance of the Saptapadi, a timeless tradition where the bride and groom intertwine their destinies by taking seven sacred vows encircling the divine fire. Each step symbolizes their unwavering dedication, love, and devotion, forging a lifelong bond that transcends time and solidifies their commitment to one another.",
      dateTime: "2024-06-18T08:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/Vivanta+Surajkund/@28.5435843,77.2614842,12.28z/data=!4m9!3m8!1s0x390cdc0daaaaaaab:0xd76f205f7679d870!5m2!4m1!1i2!8m2!3d28.4898296!4d77.2849204!16s%2Fg%2F11c71b1922?entry=ttu",
      image: "string",
    },
  ],
  pocs: [
    {
      id: 2,
      firstName: "Mohak",
      lastName: "Shivam",
      contactNumber: "8770123453",
      fromSide: "Bride",
    },
    {
      id: 1,
      firstName: "Dhruv",
      lastName: "Sumit",
      contactNumber: "97648536584",
    },
  ],
  weddingInfo: {
    id: 1,
    bride: {
      name: "Mahak Rao",
      motherName: "Geeta",
      fatherName: "Sudev",
      grandMotherName: "Varsha",
      grandFatherName: "Narayan",
      description:
        "I am  so happy to be starting a new chapter in my life with my soulmate. I know that we would face challenges together, but I am was confident that we will overcome them. I am excited to see what the future held for us.",
    },
    groom: {
      name: "Chandra Agrawal",
      motherName: "Archana",
      fatherName: "Rajesh",
      grandMotherName: "Neelam",
      grandFatherName: "Santosh",
      description:
        "I can't believe I'm getting married! I'm so excited to spend the rest of my life with the love of my life. I'm so grateful for all the love and support from our family and friends.",
    },
    thankYouMessage:
      "Thank you so much for your love and support. I am so grateful for everything you have done for me, both leading up to my wedding and on my special day. I couldn't have asked for a better wedding to share this day with. Your love and support mean the world to me. I am so lucky to have you in my life.",
    dateTime: "2024-06-18T08:29:53.796",
    pics: ["D", "E", "F"],
    placesToVisit: ["Dont know", "Don know 2"],
  },
  GalleryImg: [
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2373.JPG",
      alt: "Image 1",
      itemHeight: "67.29166666666667%",
      percentHeight: "0%",
      column: 0,
      itemsInColumn: 0,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2409.JPG",
      alt: "Image 2",
      itemHeight: "100%",
      percentHeight: "0%",
      column: 1,
      itemsInColumn: 0,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2406.JPG",
      alt: "Image 3",
      itemHeight: "100%",
      percentHeight: "17.491809735226195%",
      column: 0,
      itemsInColumn: 1,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2699.JPG",
      alt: "Image 4",
      itemHeight: "100%",
      percentHeight: "25.994020659159673%",
      column: 1,
      itemsInColumn: 1,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2719.JPG",
      alt: "Image 5",
      itemHeight: "100%",
      percentHeight: "43.48583039438586%",
      column: 0,
      itemsInColumn: 2,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2483.JPG",
      alt: "Image 6",
      itemHeight: "66.66666666666666%",
      percentHeight: "51.98804131831935%",
      column: 1,
      itemsInColumn: 2,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2650.JPG",
      alt: "Image 7",
      itemHeight: "48.541666666666664%",
      percentHeight: "69.47985105354553%",
      column: 0,
      itemsInColumn: 3,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2684.JPG",
      alt: "Image 8",
      itemHeight: "68.87052341597796%",
      percentHeight: "69.31738842442579%",
      column: 1,
      itemsInColumn: 3,
    },
    {
      image:
        "https://template-assets-sv.s3.amazonaws.com/template2/024A2596.JPG",
      alt: "Image 9",
      itemHeight: "68.87052341597796%",
      percentHeight: "82.0977819151793%",
      column: 0,
      itemsInColumn: 4,
    },
  ],

  SliderImg: [
    {
      id: 1,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2684.JPG",
    },
    {
      id: 2,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2373.JPG",
    },
  ],
  SliderImg2: [
    {
      id: 2,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2483.JPG",
    },
    {
      id: 1,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2596.JPG",
    },
  ],

  SliderImg1: [
    {
      id: 1,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2433.JPG",
    },
    {
      id: 2,
      link: "https://template-assets-sv.s3.amazonaws.com/template2/024A2406.JPG",
    },
  ],
};

export { template2InfoData };
