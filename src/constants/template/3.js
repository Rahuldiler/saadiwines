const template3InfoData = {
  images: {
    gallery4:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/7bec30e8-93aa-4ac9-8712-3c8fa0943435.JPG",
    TEST3:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/69d7e89e-eace-41b7-b737-9c79753c35fe.JPG",
    gallery5:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/53288e79-7602-4d63-b82d-0aa5e2f0f7fd.JPG",
    gallery2:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/41b9eb11-3034-4db1-9491-f7d611cb8b6d.JPG",
    gallery3:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/26378ede-0176-47b9-be03-d4573b7ebf90.JPG",
    gallery8:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/f8230d26-40ce-484b-a940-c5dc9af7c0e8.JPG",
    gallery6:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/2363c197-728a-49ff-a9e0-23e930651b7a.JPG",
    gallery7:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/2c5c8ffb-9e16-48c4-b28c-064cabebaf4a.JPG",
    date4:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/a70df2fc-9932-4226-9c6d-9d93a07c55f2.JPG",
    itinerary:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/d3453073-8910-42ae-88ac-9782784e99f0.png",
    date3:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/d93735ab-8c36-490a-ad67-a96cb1f23df9.JPG",
      
    rsvp: "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/d8947fa9-9e42-4ee8-8fb5-1fd8c4a885f9.JPG",
    itinerary3:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/f38a2fdf-86b2-4c98-8f31-5c87ae57eea4.JPG",
    itinerary4:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/d9f1db27-48e6-43db-b803-085c393eedd6.JPG",
    itinerary5:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/b96d827f-c2ee-4b14-ac27-7fcb1e9555b3.JPG",
    date2:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/615c7b86-1567-4c60-a24d-bcce4979f175.JPG",
    gallery1:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/c81febe7-8113-4346-872f-9bc06d58b43e.JPG",
    introduction:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/eb89caa4-c9ed-41b1-b0eb-3111a3ea3794.JPG",
    date1:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/2de0a9f3-c43a-40ff-bc1f-5f802508ea1f.JPG",
    itinerary2:
      "https://sv-user-images.s3.ap-south-1.amazonaws.com/102/59a14f1d-b644-4d18-8497-f1c95b3178c5.JPG",
  },
  milestone: [
    {
      id: 1,

      title:
        "What's the most adventurous activity or trip you've done as a couple?",
      description:
        "One of the most adventurous activities we did as a couple was embarking on a thrilling road trip to Leh Ladakh. Driving through the rugged terrain, navigating winding mountain roads, and crossing high-altitude passes was an exhilarating experience. We were captivated by the breathtaking landscapes, from the majestic mountains to the pristine lakes that dotted the region. It was a journey filled with adrenaline, awe-inspiring views, and unforgettable moments that we shared together.",
    },
    {
      id: 2,

      title:
        "Can you share something that holds great sentimental value in your relationship?",
      description:
        "One item that holds great sentimental value in our relationship is a small, worn-out notebook filled with handwritten notes and memories. Over the years, we've used this notebook to jot down our thoughts, dreams, and aspirations. It contains heartfelt letters, silly doodles, and cherished moments that we've shared together. Whenever we come across this notebook, we take a trip down memory lane, reminiscing about the milestones and special times we've had as a couple. It serves as a tangible reminder of our love, growth, and the beautiful journey we've embarked on together.",
    },
    {
      id: 3,

      title:
        "If you could give one piece of advice to someone starting their own love story, what would it be?",
      description:
        "One piece of advice I would give to someone starting their own love story is to prioritize open and honest communication. Communication is the foundation of a healthy and strong relationship. Be willing to express your thoughts, feelings, and desires openly, and also listen attentively to your partner. By fostering a culture of open communication, you can build trust, resolve conflicts effectively, and ensure that both partners feel heard and understood. Remember that communication is a two-way street, so make the effort to actively engage and connect with your partner on an emotional level.",
    },
  ],
  familyMembers: [
    {
      id: 1,
      name: "Yash",
      relation: "Father",
    },
    {
      id: 2,
      name: "Monika",
      relation: "Mother",
    },
    {
      id: 3,
      name: "Kapil",
      relation: "Uncle",
    },
    {
      id: 4,
      name: "Shreya",
      relation: "Sister",
    },
    {
      id: 5,
      name: "Govind",
      relation: "Brother",
    },
    {
      id: 6,
      name: "Vishal",
      relation: "Brother",
    },
  ],
  itinerary: [
    {
      id: 1,

      functionName: "The Vibrant Mehendi Ceremony",
      details:
        "Immerse yourself in the mesmerizing world of the Mehendi ceremony, where the graceful dance of intricate henna designs unfolds, embellishing the bride's hands and feet. Each delicate stroke weaves a tale, capturing the essence of boundless love, eternal bliss, and the bountiful blessings of fertility.",
      dateTime: "2024-06-18T08:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/Indira+Gandhi+International+Airport/@28.527554,77.0438314,11z/data=!4m6!3m5!1s0x390d1b85fc2a2d89:0xbef376182c43ed9d!8m2!3d28.5563529!4d77.1006963!16zL20vMDEyNDQ0?entry=ttu",
      image: "string",
    },
    {
      id: 2,
      functionName: "Joyful Sangeet Celebration",
      details:
        "Dive into the vivacious and electrifying atmosphere of the Sangeet ceremony, where music harmonies and rhythmic dances fill the air with energy. The stage ignites with mesmerizing performances, as both families join hands to honor and celebrate the union of two souls, creating an unforgettable symphony of joy and togetherness.",
      dateTime: "2024-06-18T09:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/Mazaar+Restaurant/@28.5688605,77.2328845,15z/data=!4m6!3m5!1s0x390ce3ad97db2cb5:0xdce447161ff5833!8m2!3d28.5701715!4d77.2443807!16s%2Fg%2F1v7pwvx2?entry=ttu",
      image: "string",
    },
    {
      id: 3,

      functionName: "Grandeur of Baraat",
      details:
        "Embark on a thrilling journey with the exhilarating Baraat procession, as the groom, embraced by loved ones, makes a majestic entrance to the wedding venue. The air crackles with infectious energy, music cascades through the atmosphere, and jubilant dancing sets the stage for an unforgettably vibrant celebration, where love and unity take center stage.",
      dateTime: "2024-06-18T08:29:53.796",
      address: "string",
      mapsLocation:
        "https://www.google.com/maps/place/C+R+Park+Market+No.1/@28.5350621,77.2431712,13.75z/data=!4m6!3m5!1s0x390ce3d83e0cb4b7:0xb7ece1334b216b84!8m2!3d28.5401153!4d77.2486233!16s%2Fg%2F1wk4bfx6?entry=ttu",
      image: "string",
    },
  ],
  pocs: [
    {
      id: 2,
      firstName: "Mohak",
      lastName: "Shivam",
      contactNumber: "8770123453",
      fromSide: "Bride",
    },
    {
      id: 1,
      firstName: "Dhruv",
      lastName: "Sumit",
      contactNumber: "97648536584",
    },
  ],
  weddingInfo: {
    id: 1,
    bride: {
      name: "Neha Shah",
      motherName: "Monika",
      fatherName: "Yash",
      grandMotherName: "Shivani",
      grandFatherName: "Bhavesh",
      description:
        "I am Neha, my heart brims with joy and anticipation as I prepare to marry the one who has captured my soul. I am humbled and grateful for the immense love and support bestowed upon us by our beloved family and friends.",
    },
    groom: {
      name: "Rochak Yadav",
      motherName: "Shreya",
      fatherName: "Kapil",
      grandMotherName: "Radhika",
      grandFatherName: "Manoj",
      description:
        "Greetings, everyone! Rochak here, and my heart is overflowing with joy as we gather together to celebrate this magical moment. From the depths of my romantic spirit, I cherish the dreams that have led me to this day, a day that will forever be etched in our hearts.",
    },
    thankYouMessage:
      "Thank you so much for your love and support. I am so grateful for everything you have done for me, both leading up to my wedding and on my special day. I couldn't have asked for a better wedding to share this day with. Your love and support mean the world to me. I am so lucky to have you in my life.",
    functionDateTime: "2024-06-18T08:29:53.796",
    pics: ["D", "E", "F"],
    placesToVisit: ["Dont know", "Don know 2"],
  },
};

export { template3InfoData };
