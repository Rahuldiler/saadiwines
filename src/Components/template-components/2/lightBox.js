...
const Lightbox = ({ isOpen, imageIndex, onClose, images }) => {
  ...
  function imgSlide(para) {
    if (nextImage === 0 && para === "pre") {
      setNextImage(Object.keys(images).length - 1);
    } else if (nextImage === Object.keys(images).length - 1 && para === "nxt") {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50 ">
      ...
      <div
        className="absolute cursor-pointer left-6  text-white top-1/2  transform -translate-x-1/2 -translate-y-1/2"
        onClick={() => imgSlide("pre")}
      >
        <ChevronLeftIcon className="text-white w-10 h-10" />
      </div>
      <div
        className="absolute cursor-pointer right-0  text-white  top-1/2  transform -translate-x-1/2 -translate-y-1/2"
        onClick={() => imgSlide("nxt")}
      >
